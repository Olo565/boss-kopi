@extends('layouts.app')
@section('title', 'Keranjang — BOSS KOPI')
        ...
        <div class="card">
            ...
            <div class="card-body" id="cartContent">
                <div class="text-center py-5 text-muted" id="emptyState">
                    ...
                    <a href="{{ route('pembeli.menu') }}" class="btn btn-coffee mt-3">Lihat Menu</a>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="card" id="summaryCard" style="display:none;">
            ...
            <div class="card-body">
                ...
                @auth
                @if(auth()->user()->poin_loyalitas >= 100)
                <div class="mb-3 p-3 rounded" style="background:var(--latte);">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <span class="badge badge-coffee">{{ auth()->user()->poin_loyalitas }} poin tersedia</span>
                    </div>
                    ...
                    <div class="input-group input-group-sm">
                        <input type="number" id="inputPoin" class="form-control"
                            ...
                            min="100" max="{{ auth()->user()->poin_loyalitas }}" step="100"
                ...
                <div class="mb-3 p-2 rounded" style="background:var(--latte);">
                    <small class="text-muted">
                        ...
                        Poin Anda: <strong>{{ auth()->user()->poin_loyalitas }}</strong>
...
<form id="formCheckout" action="{{ route('pembeli.checkout') }}" method="POST" style="display:none;">
    @csrf
...
let lokasiLng = null;
let poinDipakai = 0;
const MAX_POIN = {{ auth()->check() ? auth()->user()->poin_loyalitas : 0 }};
...
async function cekPromo() {
    ...
    if (!kode) return;
    const res = await fetch('{{ route("pembeli.cek-promo") }}', {
        method: 'POST',
