@extends('layouts.app')
@section('title', 'Laporan Penjualan — BOSS KOPI')
@section('page-title', 'Laporan Penjualan')
@section('page-subtitle', 'Periode: ' . \Carbon\Carbon::parse($tanggalMulai)->format('d M Y') . ' s/d ' . \Carbon\Carbon::parse($tanggalSelesai)->format('d M Y'))
    ...
    <div class="card-body">
        <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-md-4 d-flex gap-2 justify-content-end">
                <a href="{{ route('admin.laporan.penjualan.excel', request()->all()) }}"
                    class="btn btn-success">
                    <i class="bi bi-file-earmark-excel me-1"></i> Export Excel
                </a>
                <a href="{{ route('admin.laporan.penjualan.pdf', request()->all()) }}"
...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-3">
        <div class="stat-card" style="border-left-color:#2D6A4F;">
            ...
            <div class="stat-value" style="color:#2D6A4F;">{{ $orders->count() }}</div>
    ...
    <div class="col-md-3">
        <div class="stat-card" style="border-left-color:#8B2020;">
            ...
            <div class="stat-value" style="color:#8B2020;">Rp {{ number_format($totalDiskon, 0, ',', '.') }}</div>
...
<div class="row g-3 mb-4">
    <div class="col-md-4">
        <div class="card">
            ...
            <div class="card-body">
                @foreach(['tunai' => 'Tunai', 'qris' => 'QRIS', 'debit' => 'Debit/EDC'] as $key => $label)
                <div class="d-flex justify-content-between py-2 border-bottom">
                    <span class="small">{{ $label }}</span>
                    <span class="fw-600" style="color:var(--coffee);">
                        Rp {{ number_format($byMetode[$key] ?? 0, 0, ',', '.') }}
                    </span>
                </div>
                @endforeach
            </div>
            ...
            <table class="table table-hover mb-0 small">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><code>{{ $order->nomor_struk }}</code></td>
                        <td>{{ $order->kasir->name ?? 'Online' }}</td>
                        <td>
                            @php $tipeLabel = ['dine_in' => 'Dine-in', 'takeaway' => 'Takeaway', 'delivery' => 'Delivery']; @endphp
                            <span class="badge badge-latte">{{ $tipeLabel[$order->tipe_pesanan] ?? $order->tipe_pesanan }}</span>
                        </td>
                        <td>{{ $order->items->count() }} item</td>
                        <td>Rp {{ number_format($order->subtotal, 0, ',', '.') }}</td>
                        <td class="text-danger">
                            {{ $order->diskon > 0 ? '-Rp '.number_format($order->diskon, 0, ',', '.') : '-' }}
                        </td>
                        <td class="fw-600" style="color:var(--coffee);">
                            Rp {{ number_format($order->total, 0, ',', '.') }}
                        </td>
                        <td>
                            @php $pmLabel = ['tunai' => '<i class="fa fa-money-bill"></i> Tunai', 'qris' => '<i class="fa fa-qrcode"></i> QRIS', 'debit' => '<i class="fa fa-credit-card"></i> Debit']; @endphp
                            {!! $pmLabel[$order->metode_pembayaran] ?? $order->metode_pembayaran !!}
                        </td>
                    </tr>
                    @empty
                ...
                <tfoot>
                    <tr style="background:var(--latte);">
                        ...
                        <td class="fw-600">Rp {{ number_format($orders->sum('subtotal'), 0, ',', '.') }}</td>
                        <td class="fw-600 text-danger">-Rp {{ number_format($totalDiskon, 0, ',', '.') }}</td>
                        <td class="fw-700" style="color:var(--coffee);">Rp {{ number_format($totalPenjualan, 0, ',', '.') }}</td>
                        <td></td>
                    </tr>
                </tfoot>
                @endif
